...
def main():
    ...
    root_folder = os.path.join(args.savedir, "{}_{}_nocolor{}_drop{}_{}".format(
            args.model+"2dis", args.npoints, args.nocolor, args.drop, time_string))
    ...
    if args.nocolor:
        Gen, firstdis = get_model(args.model, input_channels=1, output_channels=N_CLASSES, args=args)
        _, semdis = get_model(args.model, input_channels=1, output_channels=N_CLASSES, args=args)
    else:
        Gen, firstdis = get_model(args.model, input_channels=3, output_channels=N_CLASSES, args=args)
        _, semdis = get_model(args.model, input_channels=3, output_channels=N_CLASSES, args=args)

    if args.test:
        Gen.load_state_dict(torch.load(os.path.join(args.savedir, "Gen_state_dict.pth")))
        semdis.load_state_dict(torch.load(os.path.join(args.savedir, "semdis_state_dict.pth")))
        firstdis.load_state_dict(torch.load(os.path.join(args.savedir, "firstdis_state_dict.pth")))
    ...
    if not args.test:
        ...
        optimizer_sem = torch.optim.Adam(itertools.chain(Gen.parameters(),semdis.parameters()), lr=args.lr)
        
        optimizer_firstmodel = torch.optim.Adam(itertools.chain(Gen.parameters(),firstdis.parameters()), lr=args.lr*2)
        print("Done")
        ...
        logs.flush()
        logs.write(str(optimizer_sem))
        logs.flush()
        logs.write(str(optimizer_firstmodel))
        logs.flush()
        ...
        for epoch in range(args.epochs):
            ...
            for (pts, features, seg ),(pts_trans, features_trans, seg_trans) in t:
                ...
                if epoch%3==2:
                    ...
                    x6_f, pts6_f, x5_f, pts5_f, x4_f, pts4_f, x3_f, pts3_f, x2_f, pts2_f = Gen(features_trans, pts_trans)
                    outputs_trans, class_out_trans = semdis(features_trans, pts_trans, x6_f, pts6_f, x5_f, pts5_f, x4_f, pts4_f, x3_f, pts3_f, x2_f, pts2_f)
     
                    seg_loss_trans = F.cross_entropy(outputs_trans.view(-1, N_CLASSES), seg_trans.view(-1))
                
                    # loss = seg_loss+discriminator_loss                
                    seg_loss_trans.backward()
                    optimizer_sem.step()
                ...
                optimizer_firstmodel.zero_grad()
                x6_t, pts6_t, x5_t, pts5_t, x4_t, pts4_t, x3_t, pts3_t, x2_t, pts2_t = Gen(features, pts)
                outputs, class_out = firstdis(features, pts,x6_t, pts6_t, x5_t, pts5_t, x4_t, pts4_t, x3_t, pts3_t, x2_t, pts2_t)
         
                seg_loss = F.cross_entropy(outputs.view(-1, N_CLASSES), seg.view(-1))

                seg_loss.backward()
                optimizer_firstmodel.step()
                ...
                train_loss += seg_loss.detach().cpu().item()
            ...
            if epoch%5==0:
                ...
                with torch.no_grad(): 
                    ...
                    for pts, features, seg in t:
                        ...
                        x6_t, pts6_t, x5_t, pts5_t, x4_t, pts4_t, x3_t, pts3_t, x2_t, pts2_t = Gen(features, pts)
                        outputs, _ = firstdis(features, pts,x6_t, pts6_t, x5_t, pts5_t, x4_t, pts4_t, x3_t, pts3_t, x2_t, pts2_t)
                    ...
                    if iouf>best_iou:
                        ...
                        torch.save(Gen.state_dict(), os.path.join(root_folder, "Gen_state_dict.pth"))
                        torch.save(firstdis.state_dict(), os.path.join(root_folder, "firstdis_state_dict.pth"))
                        torch.save(semdis.state_dict(), os.path.join(root_folder, "semdis_state_dict.pth"))
