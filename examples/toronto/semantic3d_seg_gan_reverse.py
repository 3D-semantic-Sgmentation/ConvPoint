...
def main():
    ...
    if not args.test:
        ...
        optimizer_Gan = torch.optim.Adam(itertools.chain(tumGen.parameters(), dis.parameters()), lr=args.lr)

        optimizer_semGen = torch.optim.Adam(semGen.parameters(), lr=args.lr)
        print("Done")
        ...
        logs.write("load pretrained models"+os.path.join("/media/liangdao/DATA/segmentation/ConvPoint/data/Prepare/SegSmall_8192_nocolorFalse_drop0.5_2022-06-01-01-03-26", "state_dict.pth"))
        ...
        logs.flush()
        logs.write("with seg trans loss in Gen, no seg loss in GAN")
        logs.flush()
        print("with seg trans loss in Gen, no seg loss in GAN")
        best_iou = 0.0
        # iterate over epochs
        for epoch in range(args.epochs):
            ...
            for (pts, features, seg ),(pts_trans, features_trans, seg_trans) in t:
                ...
                loss_gan = seg_loss+(discriminator_loss+discriminator_loss_trans)/2
                loss_gan.backward()
                optimizer_Gan.step()
