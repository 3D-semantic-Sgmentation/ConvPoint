...
def main():
    ...
    parser.add_argument('--block_size', help='Block size', type=float, default=24)
    ...
    if not args.test:
        ...
        optimizer_dis = torch.optim.Adam(dis.parameters(), lr=args.lr*2)  # Discriminator use large learning rate
        ...
        for epoch in range(args.epochs):
            ...
            for (pts, features, seg ),(pts_trans, features_trans, seg_trans) in t:
                ...
                domain_loss_trans = torch.nn.NLLLoss()(domain_class_trans.flatten(), clabel_false.long())
                ...
                outputs,domain_class = dis(features, point_features)

                clabel_true = torch.ones(domain_class.size(0)).cuda()
                
                domain_loss = torch.nn.NLLLoss()(domain_class.flatten(), clabel_true.long())
                ...
                clabel_true = torch.ones(domain_class_trans.size(0)).cuda()

                seg_loss_trans = F.cross_entropy(outputs_trans.view(-1, N_CLASSES), seg_trans.view(-1))
                domain_loss_trans = torch.nn.NLLLoss()(domain_class_trans.flatten(), clabel_true.long())
                ...
                clabel_false = torch.zeros(domain_class.size(0)).cuda()
                domain_loss = torch.nn.NLLLoss()(domain_class.flatten(), clabel_false.long())


                loss_FGNet = seg_loss_trans+(domain_loss+domain_loss_trans)
                loss_FGNet.backward()
                ...
                train_loss += seg_loss_trans.detach().cpu().item()
            ...
            if epoch%2==0:
                ...
                with torch.no_grad(): 
                    ...
                    for pts, features, seg in t:
                        ...
                        point_features = FGNet(features, pts)
                        outputs,_ = dis(features, point_features)
